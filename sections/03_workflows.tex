...
Procedure for time-series data is slighty different (Figure \ref{FIG:CLUSTER_POINTSTS}).
...
Finally, we can calculate and export distance matrix using eucidean distance (\it{tsEUC}).
...
The first step is to create a signature grid using either the {\it gpat\_gridhis} or {\it gpat_gridts} module.
The second step is to convert the obtained signature grid from binary to text format with {\it gpat_grd2txt}.
The final step is to calculate and export a distance matrix using the {\it gpat_distmtx} module (Figure \ref{FIG:CLUSTER_GRID}).
